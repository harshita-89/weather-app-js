...
function getWeatherDetails(name, lat, lon, country, state){
    let FORECAST_API_URL = `https://api.openweathermap.org/data/2.5/forecast?lat=${lat}&lon=${lon}&appid=${api_key}`,
        ...
        fetch(AIR_POLLUTION_API_URL).then(res => res.json()).then(data =>{
            let{co, no, no2, o3, so2,nh3, pm2_5, pm10} = data.list[0].components;
            aqiCard.innerHTML=`
            <div class="card-head">
        ...
        .then(data => {
            ...
            for(i =0; i<=7;i++){
                ...
                hourlyForecastCard.innerHTML += `
                    <div class="card">
                        <p>${hr} ${a}</p>
                        <img src="https://openweathermap.org/img/wn/${hourlyForecast[i].weather[0].icon}.png" alt="Clear sky icon">
...
function getusercoordinates(){
    navigator.geolocation.getCurrentPosition(position => {
        ...
        if(error.code ===error.PERMISSION_DENIED){
            alert('Geolocation permission denied. please reset location permission to gran taccess again');
        }
    });
}
